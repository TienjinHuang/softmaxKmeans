...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/ckptGA_BCE%s.t7'%(args.net+'_'+args.idx))
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckptGA_BCE%s.t7'%(args.net+'_'+args.idx))
        best_acc = acc
...
print('results are at ./checkpoint/ckptGA_BCE%s.t7'%(args.net+'_'+args.idx))
