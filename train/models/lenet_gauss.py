...
class Gauss_MV(nn.Module):
    ...
    def __init__(self,in_features,out_features,num_classes, gamma):
        ...
        self.in_features = in_features
        self.out_features = out_features
        self.Z = nn.Parameter(torch.Tensor(in_features ,out_features)) #centroids (dxc)
        self.W = nn.Parameter() # Whitening matrix (cxrxd)
        #self.weight.requires_grad=False
        #nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
    ...
    def forward(self, D):
        ...
        WZ = torch.einsum('crd,dc->cr', self.W, self.Z) # (cxr)
