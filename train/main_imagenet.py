...
def train_net(args, logger_cls):
    ...
    model_and_loss = ModelAndLoss(args,
                                  ...
                                  nn.MSELoss if args.label_smoothing == 0.0 else (
...
class ModelAndLoss(nn.Module):
    def __init__(self, args, arch, loss, pretrained_weights=None, state=None, cuda=True, fp16=False, distributed=False):
        ...
        model = models.build_resnet(arch[0], arch[1], gamma=args.gamma)
    ...
    def forward(self, data, target):
        ...
        # loss = self.loss(torch.exp(output / 16), Y)
        loss = 0.95 * self.loss(torch.exp(output / 4), Y)
        loss += 0.68 * self.loss(P1, Y)
        loss += 0.38 * self.loss(torch.exp(4 * output), Y)

        confBatch, predicted = P1.max(1)
        conf = confBatch.sum() / data.size(0)

        return loss, output, conf
...
def get_train_step(model_and_loss, optimizer, fp16):
    def _step(input, target):
        ...
        optimizer.zero_grad()
        loss, output, conf = model_and_loss(input_var, target_var)
        ...
        return reduced_loss, prec1, prec5, conf
...
def train(train_loader, model_and_loss, optimizer, fp16, logger, epoch, prof=False):
    ...
    for i, (input, target) in enumerate(prefetched_loader(train_loader, fp16)):
        ...
        loss, prec1, prec5, conf = step(input, target)
...
class EpochLogger(object):
    ...
    def iter_callback(self, epoch, iteration, d):
        ...
        self.conf.update(d['conf'], d['size'])
